...
async def login(body: LoginRequest, response: Response):
    ...
    if not verify_password(body.password, settings.nanoscope_password_hash):
        raise HTTPException(401, "Invalid password")
    token = create_session()
    response.set_cookie("nanoscope_session", token, httponly=True, samesite="strict")
    return {"ok": True}
...
async def logout(request: Request, response: Response):
    ...
    response.delete_cookie("nanoscope_session")
    return {"ok": True}
